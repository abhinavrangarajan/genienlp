...
class MultitaskQuestionAnsweringNetwork(nn.Module):
    ...
    def forward(self, batch, iteration):
        ...
        if self.args.cove:
            context_embedded = self.project_cove(
                torch.cat([self.cove(context_embedded[:, :, -300:], context_lengths), context_embedded], -1).detach())
            ...
            for b in range(context.size(0)):
                lc = [self.field.decoder_itos[i] if i < self.args.max_generative_vocab else self.field.decoder_itos[0]
                      for i in context[b, :]]
            ...
            for b in range(question.size(0)):
                lq = [self.field.decoder_itos[i] if i < self.args.max_generative_vocab else self.field.decoder_itos[0]
                      for i in question[b, :]]
            ...
            context_embedded = self.project_elmo(torch.cat(
                [self.elmo(batch_to_ids(context_list).to(self.device))['elmo_representations'][0], context_embedded], -1).detach())
            question_embedded = self.project_elmo(torch.cat(
                [self.elmo(batch_to_ids(question_list).to(self.device))['elmo_representations'][0], question_embedded], -1).detach())
    ...
    def greedy(self, self_attended_context, context, question, context_indices, question_indices, oov_to_limited_idx,
        ...
        for t in range(T):
            ...
            probs = self.probs(self.out, rnn_output, vocab_pointer_switch, context_question_switch,
                               context_attention, question_attention,
                               context_indices, question_indices,
                               oov_to_limited_idx)  # size  (29, 1, vocab_size)
    ...
    def scoring(self, self_attended_context, context, question, context_indices, question_indices, oov_to_limited_idx, rnn_state=None):
        B, TC, C = context.size()
        T = self.args.max_output_length
        scores = context.new_full((B, T), 0.5, dtype=torch.float)
        outs = context.new_full((B, T), self.field.decoder_stoi['<pad>'], dtype=torch.long)
        hiddens = [self_attended_context[0].new_zeros((B, T, C))
                   for l in range(len(self.self_attentive_decoder.layers) + 1)]
        hiddens[0] = hiddens[0] + positional_encodings_like(hiddens[0])
